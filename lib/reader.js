...
var readAll = function (filePath, frontMatterConfig, partnerFile) {
    ...
    let slugEnd = canonical ? canonical.split('/').pop() : '';

    const title = $('.p-name').text();
    const subtitle = $('.p-summary[data-field="subtitle"]').text();    
    const draft = isDraft ? '1' : '';
    const partnered = isDraft ? '' : checkPartnered(slugEnd, partneredPosts) ? '1' : '';
    const submitted = ''; // not in use currently
    const publication = isDraft || !partnered ? '' : '2Easy 2Publication'
    ...
    const frontMatter = generateFrontMatter(title, subtitle, date, slugMedium, partnered, submitted, draft, publication);
...
var generateFrontMatter = function (title, subtitle, date, slugMedium, partnered, submitted, draft, publication) {
    ...
    frontMatter.date = date ? date.toString() : (new Date()).toISOString();
    frontMatter.slugMedium = slugMedium ? slugMedium.toString() : '';
    frontMatter.partnered = partnered ? partnered : '';
    frontMatter.submitted = submitted ? submitted : '';
